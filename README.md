...
매일 오전 10시(KST) GitHub Actions가 자동으로 아래 순서로 실행됩니다.

```
1. 텔레그램 명령 수집 → 명령어 처리 및 응답
...
봇에게 아무 메시지를 보낸 뒤, 아래 URL로 확인합니다.
...
이후 매일 오전 10시(KST)에 자동 실행됩니다. `workflow_dispatch`로 수동 실행도 가능합니다.
...
명령은 다음 번 Actions 실행(오전 10시 KST) 때 처리됩니다.
...
허용(`/allow`)되지 않은 사용자는 `/help` 외 모든 기능이 제한되며, 대신 본인의 chat_id가 안내됩니다.
...
명령어는 다음 Actions 실행(오전 10시 KST) 때 일괄 처리됩니다.
즉시 응답을 원하면 Actions 탭에서 `workflow_dispatch`로 수동 실행하세요.

### 봇에 메시지를 보냈는데 chat_id 안내가 왔어요

관리자가 `/allow <chat_id>`로 허용해 주어야 봇을 사용할 수 있습니다.
안내받은 chat_id를 관리자에게 전달하세요.
...
4. `/status` 명령으로 알림 활성화(`active: 활성`) 상태인지 확인하세요.
