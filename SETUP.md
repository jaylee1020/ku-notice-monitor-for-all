...
4. 다음 Actions 실행 때 시작 안내 메시지가 자동으로 옵니다

> 명령어는 즉시 처리되지 않고 **다음 오전 10시 실행 때 일괄 처리**됩니다.
> 바로 확인하려면 Actions 탭 → `Run workflow`로 수동 실행하세요.
...
**명령어를 보냈는데 응답이 없어요**
다음 오전 10시 실행 때 처리됩니다. 빨리 확인하려면 Actions 탭에서 수동 실행하세요.
